...
LastEditTime: 2022-06-25 07:45:49
...
from models import get_down_model
...
def compute_speed(model, input_size, device, iteration, imgpath):
    ...
    for _ in range(50):
        ...
        input = tfs(img).view(1,3,224,224).cuda()
        model(input)
        torch.cuda.synchronize()
    ...
    for _ in range(iteration):
        ...
        with torch.no_grad():
            ...
            com1 = pred.cpu().data.sigmoid()>0.5
        torch.cuda.synchronize()
...
def train_downstream_method(gpu_id, cfg_pth):
    ...
    model = get_down_model(num_classes_cls=[12, 13, 6, 9])
    
    total = sum([param.nelement() for param in model.parameters()]) 
    print("Number of parameter: %.2fM\n" % (total/1e6))


    # net = TestModel(truncation, args.temp, args.alpha0, args.hidden, num_classes, 3, 224)
    print("\nComputing speed:")
    device = 1
    imgpath = '/media/data/sy_data/DataSet/CelebA/img_align_celeba/141820.jpg'
    compute_speed(model, (1,3,224,224), device, 1000, imgpath)
...
if __name__ == '__main__':
    ...
    train_downstream_method('1', cfg_pth)
