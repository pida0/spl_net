...
LastEditTime: 2022-06-25 09:54:31
...
def train_pretext_method(gpu_id, cfg_pth):
    ...
    #         sum([param.nelement() for param in net_D.parameters()]) + 
    #         sum([param.nelement() for param in net_Rot.parameters()]))
    # print("Number of parameter: %.2fM\n" % (total/1e6))


    # 测Params和FLOPs
    # stat(model, (27, 224, 224))
    # stat(net_D, (3, 224, 224))

    
    # 测MACS和Params
    input = torch.randn(1, 27, 224, 224)
    # input.cuda()
    # model.cuda()
    flops, params = profile(model, inputs=(input,))
    # print(flops/(1000**3)) # MACS
    # print(params/(1000**2))
    ...
    #         prt_out, pst_out, pct_out, x_psct_fake, x_psct_real, x_prtg_rot = model(imgs[0], imgs[1])
    #         x_psct_real_logit = x_psct_real.detach()
    #         x_psct_fake_logit = x_psct_fake.detach()
