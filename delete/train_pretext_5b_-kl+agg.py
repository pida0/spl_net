'''
Author: sy
Date: 2022-06-04 16:31:20
LastEditors: pida0
LastEditTime: 2022-07-12 16:12:22
...
LastEditTime: 2021-04-19 13:51:11
...
from models import get_model_5b, Discriminators, get_model_5b_agg
...
def train_pretext_method(gpu_id, cfg_pth):
    ...
    model = get_model_5b_agg(num_classes_rot=cfg.DATA.PATCH_NUM,
                      num_classes_seg=19)
    ...
    for epoch in range(0, cfg.TRAIN.N_EPOCH):
        ...
        for batch_idx, (imgs, labels) in enumerate(trainloader):
            ...
            prt_out, pst_out, pct_out = model(imgs[0], imgs[1])
            ...
            loss = pst_loss + pct_loss + prt_cls_loss 
            optimizer.zero_grad()
            # d_optimizer.zero_grad()
            loss.backward()
            ...
            if batch_idx % (print_inter) == 0:
                ...
                print('[Loss]: pst_loss: %.4f, pct_loss: %.4f, prt_cls_loss: %.4f, total_loss: %.4f'
                      % (pst_loss, pct_loss, prt_cls_loss, loss))
